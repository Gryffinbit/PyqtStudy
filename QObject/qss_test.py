...
class Window(QWidget):
    ...
    def QObject信号的操作(self):
     ...
     def QObject类型判定(self):
        ...
        # label = QLabel()
        # btn = QPushButton()
        # objs = [obj,w,label,btn]
        # for o in objs:
        #     print(o.isWidgetType())   # 判断对象的类型
        #     print(o.inherits("QWidget"))       # 判断一个对象是否继承（直接或间接）自某个类
        # *************API***************结束
        ...
        btn = QPushButton(self)
        btn.setText("fuck you!")
        btn.move(250, 250)

        for widget in self.findChildren(QLabel):    # self. 代表着，这个界面下面的所有子控件
            widget.setStyleSheet("background-color:blue;")    # 所有控件变成蓝色。如果是self.setStyleSheet 则是整个界面都变成蓝色
        ...
        # for widget in self.children():
        #     if widget.isWidgetType():   # btn, label都是继承了widget
        #         widget.setStyleSheet("background-color:blue;")
